...
export function PlatformMenu({ open }: { open: boolean }) {
  return (
    ...
    >
      <div className="grid w-[min(90vw,52rem)] grid-cols-1 gap-8 rounded-2xl border border-border bg-card p-7 shadow-overlay sm:grid-cols-3">
        {columns.map((col) => (
          <div key={col.title}>
                      ...
                      : name === "Bokningsassistent"
                        ...
                        : name === "Bordsplacering"
                          ...
                          : name === "Salsplan"
                            ...
                            : name === "Tillgänglighet"
                              ? ({ to: "/tillganglighet" } as const)
                              : name === "Bokningsregler"
                                ? ({ to: "/bokningsregler" } as const)
                                : name === "Gästinsikt"
                                  ? ({ to: "/gastinsikt" } as const)
                                  : name === "Analys"
                                    ? ({ to: "/analys" } as const)
                                    : name === "Kampanjer"
                                      ? ({ to: "/kampanjer" } as const)
